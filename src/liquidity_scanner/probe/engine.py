...
class ProbeEngine:
    ...
    async def probe_asset(self, asset: Asset) -> ProbeResult:
        now = datetime.now(UTC)
        session_state = get_session_state(now)
        limiter = TokenBucketRateLimiter(self.settings.jup_rps)
        async with httpx.AsyncClient() as http_client:
            jup = JupiterClient(self.settings, http_client, limiter)
            ...
            quote_meta = await jup.get_prices([quote_mint])
            quote_decimals = quote_meta.get(quote_mint, meta).decimals
            ...
            base_out, _ = await self._median_quote_usd(
                jup, asset.mint, self.settings.usdc_mint, base_raw, 6, samples=1
            )
            ...
            for notional in self.settings.ladder_notionals_usd:
                ...
                if asset.kind == "launch" and asset.quote_mint:
                    ...
                    if leg1_raw_out is not None:
                        leg1_out = leg1_raw_out
                        quote_px = quote_meta.get(quote_mint)
                        if quote_px:
                            leg1_usd = leg1_out * quote_px.usd_price
                        leg2_raw = raw_amount(leg1_out, quote_decimals)
    ...
    async def _bisect_cliff(
        ...
        for _ in range(iterations):
            ...
            qty = mid / ref_price
            amount_raw = raw_amount(qty, decimals)
            out, route = await self._median_quote_usd(jup, mint, self.settings.usdc_mint, amount_raw, 6, samples=3)
            if out is None:
                hi = mid
                continue
            if baseline_exec_px:
